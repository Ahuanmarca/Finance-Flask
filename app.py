...
def index():
    ...
    user_portfolio = db.execute("SELECT * FROM portfolios JOIN stocks ON portfolios.stock_id = stocks.id WHERE portfolios.user_id = ? ORDER BY stocks.symbol;", session["user_id"])
    ...
    return render_template("index.html", user_portfolio=user_portfolio, cash=cash, grand_total=grand_total)
...
def buy():
    ...
    if request.method == "POST":
        ...
        price = float(business_info["price"]) # Convertido en float porque viene como string de lookup... creo...
        ...
        if not stored_stock:
            stock_id = db.execute("INSERT INTO stocks(symbol, name) VALUES(?, ?);", symbol, name)
        ...
        # Si ya existe symbol en stocks, debo chequear si user tiene shares en portfolio. No tiene: INSERT. Si tiene: UPDATE.
            ...
            owned_stock = db.execute("SELECT * FROM portfolios WHERE user_id=? AND stock_id=?;", session["user_id"], stock_id)
            ...
            # User SI TIENE shares
                ...
                db.execute("UPDATE portfolios SET shares=? WHERE user_id=? AND stock_id=?;", total_shares, session["user_id"], stock_id)
...
def history():
    ...
    user_history = db.execute("SELECT * FROM transacciones WHERE user_id=?;", session["user_id"])

    # Hacer loop para agregar symbol a cada row (dict) de user_history
    for row in user_history:
        row["symbol"] = db.execute("SELECT * FROM stocks WHERE id=?;", row["stock_id"])[0]["symbol"]
...
def register():
    ...
    if request.method == "POST":
        ...
        if not username: return apology("missing username", 400)
        if not password: return apology("missing password", 400)
        if not passConf: return apology("missing password confirmation", 400)
        if password != passConf: return apology("invalid password confirmation", 400)
...
def sell():
    ...
    if request.method == "POST":
        ...
        stored_stock = db.execute("SELECT * FROM stocks WHERE symbol=?;", selling_symbol)
        if not stored_stock:
            return apology("no stocks to sell", 400)

        # Chequear si existe stock en portfolio de user
        owned_stock = db.execute("SELECT * FROM portfolios WHERE user_id=? AND stock_id=?;", session["user_id"], stored_stock[0]["id"])
        if not owned_stock:
            return apology("no stocks to sell", 400)
        ...
        if total_shares > 0:
            db.execute("UPDATE portfolios SET shares=? WHERE user_id=? AND stock_id=?;", total_shares, session["user_id"], stored_stock[0]["id"])
        # Pero si nuevo total es cero shares, eliminar row de portfolio de usuario
        else:
            db.execute("DELETE FROM portfolios WHERE user_id=? AND stock_id=?;", session["user_id"], stored_stock[0]["id"])
        ...
        db.execute("INSERT INTO transacciones(user_id, stock_id, shares, price, p_datetime) VALUES(?, ?, ?, ?, ?);", session["user_id"], stored_stock[0]["id"], selling_shares * -1, current_price, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        ...
        user_portfolio = db.execute("SELECT * FROM stocks WHERE id IN (SELECT stock_id FROM portfolios WHERE user_id=?);", session["user_id"])
